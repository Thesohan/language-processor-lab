...
def parseString(input):
 	...
 	stack.append('$')
 	stack.append(startingSymbol)
 	while(True):
 		#break
 		print(stack,input[currentInputPointer:],sep='\t')
		
 		if (input[currentInputPointer]==stack[-1] and stack[-1]=='$'):
 			print("parsing successful")
 			break
 		elif (input[currentInputPointer]==stack[-1]):
 			stack.pop(-1)
 			currentInputPointer+=1
 		elif (input[currentInputPointer]<='Z'and input[currentInputPointer]>='A'):
 				break
 		else:
 			prod=parseTable.get(stack[-1]+input[currentInputPointer])
 			if(prod!=None):
 				if(len(stack)>0):
 					stack.pop(-1)
 				finalProd=prod[1:]
 				for symbol in finalProd[::-1]:
 					if(symbol=='e'):
 						continue
 					stack.append(symbol)
 			else:
